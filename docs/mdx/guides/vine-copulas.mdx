...
A float in `[0, 1]`. When set, an edge whose dependence measure (Kendall's tau by default, or the configured `tree_criterion`) has absolute value at or below this cut-off is assigned the independence copula without running family selection. This is a raw cut-off, not a statistical test.

### `independence_test_level`

A significance level in `(0, 1)`, for example `0.05`. When set, each edge is first checked with the asymptotic Kendall's tau independence test (the same test vinecopulib uses); edges where the test does not reject independence at this level are assigned the independence copula. This adapts to the sample size, unlike the raw threshold, and is the recommended way to sparsify a vine. Both options can be combined.
