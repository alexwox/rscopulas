...
The default `fit_method="composite_mle"` optimizes pairwise scores at each node. `recursive_mle` is a compatibility alias. `tau_init` uses Kendall-tau inversion without optimization. `full_mle`, `smle`, and `dmle` raise an explicit unsupported-method error. No simulated likelihood is performed: `used_smle` is false and `mc_samples` must be zero (its default); nonzero values are rejected. `allow_experimental=False` rejects mixed-family fitted trees.
