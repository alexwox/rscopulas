...
<AccordionGroup>
  <Accordion title="rscopulas.to_pseudo_obs (built-in rank transform)">
    The package ships a NumPy-only helper that performs the rank-based transform for you, handles ties, and rejects NaN with a clear error. It accepts a 1-D vector or an `(n, d)` matrix and returns `float64` values strictly inside `(0, 1)`.
    ...
    U = to_pseudo_obs(X)                     # rank / (n + 1), ties averaged
    U_min = to_pseudo_obs(X, ties="min")     # or "max", "ordinal"
    U_hazen = to_pseudo_obs(X, scaling="n")  # (rank - 0.5) / n
    fit = GaussianCopula.fit(U)
    ```

    `ties` follows `scipy.stats.rankdata` semantics (`average`, `min`, `max`, `ordinal`) without requiring SciPy. NaN entries raise `InvalidInputError` naming the first offending row and column; `±inf` are ranked at the ends.
  </Accordion>
...
In Python, invalid input raises `InvalidInputError` (a subclass of both `RscopulasError` and `ValueError`). `to_pseudo_obs` never produces boundary values; if you build the matrix yourself, check it before calling `.fit()`:
