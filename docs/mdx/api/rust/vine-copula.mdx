...
pub struct VineFitOptions {
    ...
    pub independence_threshold: Option<f64>,  // raw cut-off: independence if |dependence| <= threshold
    pub independence_test_level: Option<f64>, // asymptotic Kendall tau test at this significance level
    // plus tree_criterion, tree_algorithm, select_trunc_lvl, rng_seed; see the crate docs
}
```
...
| `Frank` | Single `theta != 0`; negative values model negative dependence. No rotation needed. |
| `Gumbel` | Single `theta >= 1`. Rotations available. |
| `Joe`, `Bb1`, `Bb6`, `Bb7`, `Bb8`, `Tawn1`, `Tawn2` | One- and two-parameter Archimedean and extreme-value families. Rotations available; two-parameter families are fitted by joint maximum likelihood. |
| `Tll` | Nonparametric transformation local likelihood. |
| `Khoudraji` | Asymmetric composition of two base copulas. Not in the default family set because it is expensive; opt in explicitly. |

**`SelectionCriterion` variants:** `Aic`, `Bic`.

The default `VineFitOptions` uses the family set Independence, Gaussian, StudentT, Clayton, Frank, Gumbel, Joe, Bb1 and Bb7, enables rotations, uses AIC, and applies no truncation or independence test. All pair fits are maximum-likelihood estimates; `FitOptions::max_iter` caps the optimiser iterations.
