...
| `InvalidInputError` | Also a `ValueError`. Input data or arguments fail validation: wrong shape, values outside `(0, 1)`, a dimension that does not match the model at evaluation time, an unknown family/rotation/criterion/method string, an invalid seed or sample count, or malformed JSON passed to `from_json`. |
| `ModelFitError` | The optimization algorithm fails to find a valid solution, or the fitting configuration is unsupported by the core (e.g. a non-Kruskal tree algorithm for a C-vine). |
...
The full hierarchy (`InvalidInputError` is also a `ValueError`; `NonPrefixConditioningError` derives from it) is listed on the [package overview](/api/python/overview#error-types).

## Objective type

`diagnostics.likelihood_kind` is `"joint"` for ordinary joint likelihoods.
For nested HAC fits it is `"composite"`: `loglik` is a pairwise score and
ordinary `aic`/`bic` are NaN. Nested HAC joint density is not implemented;
call `model.composite_log_pdf(data)` to evaluate the score explicitly.
Factor and HAC iteration counts record completed work. A false `converged`
flag indicates that the stopping condition was not reached (or factor
joint polishing was disabled).
