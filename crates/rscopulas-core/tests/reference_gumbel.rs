...
use rscopulas::{
    CopulaModel, EvalOptions, FitOptions, GumbelHougaardCopula, PseudoObs, SampleOptions,
};
...
#[derive(Debug, Deserialize)]
struct GumbelSampleSummaryFixture {
    metadata: Metadata,
    theta: f64,
    seed: u64,
    sample_size: usize,
...
#[test]
fn gumbel_log_pdf_matches_r_fixture() {
...
#[test]
fn gumbel_fit_matches_r_fixture() {
    let fixture: GumbelFitFixture = load_fixture("gumbel_fit_d2_case01.json");
    assert_eq!(fixture.metadata.source_package, "copula");
    let input =
        PseudoObs::new(array2(&fixture.input_pobs)).expect("fixture inputs should be valid");

    let fit =
        GumbelHougaardCopula::fit(&input, &FitOptions::default()).expect("fit should succeed");
    assert!(
        (fit.model.theta() - fixture.expected_theta).abs() < 1e-10,
...
#[test]
fn gumbel_sample_statistics_match_r_fixture() {
    let fixture: GumbelSampleSummaryFixture = load_fixture("gumbel_sample_summary_d2_case01.json");
    assert_eq!(fixture.metadata.source_package, "copula");
    let model = GumbelHougaardCopula::new(2, fixture.theta).expect("fixture theta should be valid");
