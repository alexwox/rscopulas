...
#[derive(Debug, Deserialize)]
struct ClaytonSampleSummaryFixture {
    metadata: Metadata,
    theta: f64,
    seed: u64,
    sample_size: usize,
...
#[test]
fn clayton_log_pdf_matches_r_fixture() {
...
#[test]
fn clayton_fit_matches_r_fixture() {
    let fixture: ClaytonFitFixture = load_fixture("clayton_fit_d2_case01.json");
    assert_eq!(fixture.metadata.source_package, "copula");
    let input =
        PseudoObs::new(array2(&fixture.input_pobs)).expect("fixture inputs should be valid");

    let fit = ClaytonCopula::fit(&input, &FitOptions::default()).expect("fit should succeed");
    assert!(
...
#[test]
fn clayton_sample_statistics_match_r_fixture() {
    let fixture: ClaytonSampleSummaryFixture =
        load_fixture("clayton_sample_summary_d2_case01.json");
    assert_eq!(fixture.metadata.source_package, "copula");
    let model = ClaytonCopula::new(2, fixture.theta).expect("fixture theta should be valid");
