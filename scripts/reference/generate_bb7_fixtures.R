...
# Generate reference fixtures for the BB7 pair copula (VineCopula family 9).
# Run from repo root.
...
# BB7: θ ≥ 1, δ > 0. Rotations 29/39 expect both parameters negated.
cases <- list(
...
for (case in cases) {
  par_stored <- if (case$family %in% c(29, 39)) abs(case$par) else case$par
  par2_stored <- if (case$family %in% c(29, 39)) abs(case$par2) else case$par2
  fixture <- list(
    metadata = metadata,
    family = case$name,
