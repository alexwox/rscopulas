...
write_fixture("gaussian_log_pdf_d2_case01.json", log_pdf_fixture)

set.seed(20260418)
fit_input <- rCopula(256, cop)
fit_model <- fitCopula(normalCopula(dim = 2, dispstr = "un"), fit_input, method = "itau")
fit_rho <- as.numeric(coef(fit_model))
fit_fixture <- list(
  metadata = metadata,
  input_pobs = to_rows(fit_input),
  expected_correlation = list(c(1.0, fit_rho), c(fit_rho, 1.0))
)
write_fixture("gaussian_fit_d2_case01.json", fit_fixture)
...
write_fixture("gaussian_sample_summary_d2_case01.json", sample_fixture)
