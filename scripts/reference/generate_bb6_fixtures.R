...
# Generate reference fixtures for the BB6 pair copula (VineCopula family 8).
# Run from repo root.
...
# BB6: θ ≥ 1, δ ≥ 1. Rotations 28/38 expect both parameters negated.
cases <- list(
  list(name = "bb6", family = 8, par = 2.0, par2 = 1.5),
  list(name = "bb6_rot90", family = 28, par = -2.0, par2 = -1.5),
  list(name = "bb6_rot180", family = 18, par = 2.0, par2 = 1.5),
  list(name = "bb6_rot270", family = 38, par = -2.0, par2 = -1.5)
)

for (case in cases) {
  par_stored <- if (case$family %in% c(28, 38)) abs(case$par) else case$par
  par2_stored <- if (case$family %in% c(28, 38)) abs(case$par2) else case$par2
  fixture <- list(
    metadata = metadata,
    family = case$name,
