...
write_fixture("gumbel_log_pdf_d2_case01.json", log_pdf_fixture)

set.seed(20260430)
fit_input <- rCopula(256, cop)
fit_model <- fitCopula(gumbelCopula(dim = 2), fit_input, method = "itau")
fit_fixture <- list(
  metadata = metadata,
  input_pobs = to_rows(fit_input),
  expected_theta = as.numeric(coef(fit_model))[1]
)
write_fixture("gumbel_fit_d2_case01.json", fit_fixture)
...
write_fixture("gumbel_sample_summary_d2_case01.json", sample_fixture)
